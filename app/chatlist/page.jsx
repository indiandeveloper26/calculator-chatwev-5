...
// Avatar color generator (theme independent)
function stringToColor(str) {
  let hash = 0;
  for (let i = 0; i < str.length; i++) {
    hash = str.charCodeAt(i) + ((hash << 5) - hash);
    ...
    >
      ...
      <div className="flex-1 overflow-y-auto space-y-3">
        ...
        {visibleChats.map((item) => {
          ...
          const firstLetter = user.charAt(0).toUpperCase();
          const isOnline = onlineUsers.includes(user);
          const avatarBg = stringToColor(user);

          return (
            <div
              key={user}
              className={`flex items-center justify-between rounded-xl p-3 md:p-4 cursor-pointer transition
              ${theme === "dark"
            ...
            >
              ...
              >
                ...
                >
                  ...
                  {/* Online dot */}
                  <span
                    className={`absolute bottom-0 right-0 w-4 h-4 md:w-5 md:h-5 rounded-full border-2
      ...
      {modalVisible && (
        ...
        >
          <div
            className={`p-6 rounded-2xl w-11/12 max-w-sm
